...
const Progress = ({ chaptersTotal, chaptersFinished }) => {
  ...
  return (
    <div className="logicPreface">
      ...
      <div style={{ maxHeight: !show ? "0px" : "500px" }} className="progress">
        {progressColors.map((i, pos) => (
          <div
            key={pos}
            className="progressTile"
...
const Course = ({ name, author, edition, status, note, link, progress }) => {
  ...
  return (
    ...
    >
      ...
      <div className="status">{text_status_map(status)}</div>
  ...
  return (
    <div>
      <header>
        ...
        <p>
          When Gene got free time he usualy does maths (or something
          CS-related). His studies are partitioned into courses, and each course
          is based on a book. Each course is either finished, in progress, or in
          a semi-permanent hiatus. Solutions for (some of) the exercies in the
          books are scrupulously written down, and presented for your viewing
          pleasure.
        </p>
        <p>
          Given that no one on the God's green earth (except for the Mighty Gene
          himself) has seen or read any of those solutions, it goes without
          saying that they are riddled with errors.
        </p>
        <p>
          This page has a related{" "}
          <a href={links.exercises}>github repository</a>. If those solutions
          helped you in any capacity, you can go there and give it a star.
        </p>
      </header>

      {Array.isArray(list) && list.length > 0 ? (
        <AllCourses list={list} lastUpdate={lastUpdate} />
      ) : (
        <Status list={list} />
      )}
    </div>
